...
# cat and monkey are both animals so it follows that they are the most similar pair
# monkeys are more well-known for eating bananas compared to cat so it follows that bananas are more closely affiliated with monkeys than cats



#==== example file with en_core_web_md ====
nlp = spacy.load('en_core_web_md')
...
# web_md tends to rate sentences as more similar compared to web_sm
# my IDE throws up a UserWarning while using web_sm saying:
# The model you're using has no word vectors loaded, 
# so the result of the Doc.similarity method will be 
# based on the tagger, parser and NER, which may not 
# give useful similarity judgements. This may happen 
# if you're using one of the small models, e.g. `en_core_web_sm`, 
# which don't ship with word vectors and only use 
# context-sensitive tensors. You can always add your own 
